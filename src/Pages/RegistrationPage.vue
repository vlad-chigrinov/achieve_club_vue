...
async function sendProofCode() {
  if (validateInputs()) {
    await axios
      .post('https://achieve.by:5000/api/email/proof_email', emailAddress.value, {
      ...
      .catch((e) => {
        if (e.response.data == 'email') {
          registrationError.value = 'Пользователь с такой почтой уже зарегистрирован'
        } else if (e.response.data == 'timeout') {
          OpenModal
        }
      })
  }
